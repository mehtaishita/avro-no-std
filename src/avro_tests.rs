use crate::{avro, avro_validation, types::*};
...
fn test_fingerprint_raw() {
	for (raw_schema, expected) in PRIMITIVE_EXAMPLE {
		let schema_result = avro_validation::fingerprint_raw_schema(raw_schema);
		if *expected {
			assert!(
				schema_result.is_ok(),
...
fn test_valid_cast_to_string_after_parse() {
	for (raw_schema, expected) in VALID_EXAMPLES {
		...
		if *expected {
			...
			let translate_schema = avro_validation::translate_schema(schema_res.1);
			assert!(
...
fn test_get_writer_with_schema() {
	let schema_result = avro_validation::fingerprint_raw_schema(r#"{"type": "int"}"#);
...
fn test_set_writer_with_data() {
	...
	assert!(translate_schema.is_ok());
	let translated_schema = translate_schema.unwrap();
	let mut writer = avro_validation::get_schema_data_writer(&translated_schema);
	assert_eq!(writer.schema(), &translated_schema);
	// the Record type models our Record schema
	let mut record = Record::new(writer.schema()).unwrap();
	record.put("a", 27i64);
...
fn test_invalid_translation() {
	...
	let schema_result = avro_validation::translate_schema(bad_bytes);
	assert!(
		schema_result.is_err(),
		"schema {} was supposed to be invalid; error: {:?}",
		bad_schema,
